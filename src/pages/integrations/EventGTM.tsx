...
import { Container, Row, Col, Card, Button, Offcanvas, Badge, Accordion, Table, InputGroup, Modal, Form, Spinner } from "react-bootstrap";
...
import SampleGTMExportOriginal from "../../data/JSONFiles/SampleGTMExportOriginal.json";
...
const EventGTM = () => {
            ...
            <Row>
              <Col className='relative inline-block'>
                <InputGroup className='my-2'>
                  ...
                  <InputGroup.Text className='p-0 rounded-end'>
                    <Button size='sm' variant='outline-info' onClick={() => checkEventIntegration(eventUrls[event.tagId]?.url || "", event.tagId)} disabled={!eventUrls[event.tagId]?.isValid || eventTests[event.tagId]?.isLoading} className='border-0 h-100'>
                      {eventTests[event.tagId]?.isLoading ? (
                        <>
                          <span className='spinner-border spinner-border-sm me-1' role='status' aria-hidden='true'></span>
                          Testing...
                        </>
                ...
                {eventTests[event.tagId]?.error && (
                  <div className='alert alert-danger mt-2' role='alert'>
                    <small>{eventTests[event.tagId]?.error}</small>
                  </div>
                )}
                {eventTests[event.tagId]?.response?.success && (
                  <div className='alert alert-success mt-2' role='alert'>
                    <small>Event check successful! Found {(eventTests[event.tagId]?.response?.events || []).length} matching events.</small>
                  </div>
                )}
              </Col>
            </Row>
  ...
  const handleDownloadGTMExport = () => {
    try {
      setIsDownloading(true);

      // Create JSON blob
      const jsonString = JSON.stringify(SampleGTMExportOriginal, null, 2);
      ...
      // Show video modal after successful download with a delay
      setTimeout(() => {
        setIsDownloading(false);
        setShowVideoModal(true);
      }, 1500); // 1.5 second delay
    } catch (error) {
      console.error("Error downloading GTM Export:", error);
      setIsDownloading(false);
      alert("There was an error downloading the GTM Export. Please try again.");
    }
  };
  ...
  const renderVideoModal = () => (
    <Modal show={showVideoModal} onHide={handleCloseVideoModal} size='lg' centered className='event-gtm-modal'>
      <Modal.Header closeButton>
        <Modal.Title>How to Import GTM Export File</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <div className='ratio ratio-16x9'>
          <iframe src='https://www.youtube.com/embed/f2YxthnDoV8' title='How to Import GTM Export File' allowFullScreen allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture' />
        </div>
      </Modal.Body>
      <Modal.Footer>
  ...
  const handleDownloadDataLayerGuide = () => {
    try {
      fetch("/src/data/JSONFiles/DataLayer Guide.txt")
        ...
        .then((content) => {
          ...
          const link = document.createElement("a");
          link.href = url;
          link.download = "DataLayer_Guide.txt";
        ...
        .catch((error) => {
          console.error("Error downloading DataLayer Guide:", error);
          alert("There was an error downloading the DataLayer Guide. Please try again.");
        });
    } catch (error) {
      console.error("Error downloading DataLayer Guide:", error);
      alert("There was an error downloading the DataLayer Guide. Please try again.");
  ...
  return (
    <Container fluid className='p-6'>
      ...
      <div className='d-grid gap-2 d-md-flex justify-content-md-end my-2'>
        <Button onClick={handleDownloadGTMExport} className='me-2' disabled={isDownloading}>
          {isDownloading ? (
            <>
        ...
        <Button onClick={handleDownloadDataLayerGuide} variant='outline-primary'>
          <i className='fe fe-file-text me-1'></i>
          Download DataLayer Guide
        </Button>
      </div>
    </Container>
  );
