...
import { Button, Modal, Drawer, Table, Form, Input, Row, Col, Card, Space, Alert, Typography, message, Tooltip } from "antd";
    ...
    <div style={{ padding: 24, background: "#f6f8fa", minHeight: "100vh" }}>
      ...
      <Form layout='inline' style={{ marginBottom: 24 }}>
        <Form.Item label='Domain' required>
          <Input style={{ minWidth: 300 }} placeholder='Domain giriniz (örn: www.example.com.tr)' value={userDomain} onChange={(e) => setUserDomain(e.target.value)} />
        </Form.Item>
      </Form>
      <Row gutter={[24, 24]}>
        {eventList.map((ev) => (
        ...
        <Col xs={24} sm={12} md={8} lg={6}>
          <Card hoverable style={{ minHeight: 140, textAlign: "center", color: "#1890ff", border: "1px dashed #1890ff", borderRadius: 12, background: "#f0f5ff" }} bodyStyle={{ padding: 18 }} onClick={handleAddCustomEvent}>
            <Input style={{ marginBottom: 8 }} placeholder='Yeni event adı' value={customEventName} onChange={(e) => setCustomEventName(e.target.value)} onPressEnter={handleAddCustomEvent} />
      ...
      <Modal
        ...
        {selectedEvent && (
          <>
            <Table dataSource={eventParams[selectedEvent].map((p) => ({ ...p, key: p.key }))} columns={columns} pagination={false} style={{ marginBottom: 16, background: "#fff", borderRadius: 8 }} rowClassName={(record) => (record.default ? "default-param-row" : "")} />
            ...
            <div style={{ margin: "24px 0" }}>
              ...
              <Paragraph copyable style={{ background: "#fff", padding: 12, borderRadius: 6 }}>
                {buildRequestUrl()}
              </Paragraph>
            </div>
            {result && <Alert message={"error" in result ? "Hata" : "Başarılı"} description={"error" in result ? result.error : JSON.stringify(result, null, 2)} type={"error" in result ? "error" : "success"} showIcon />}
          </>
        )}
        <Drawer title={editIndex !== null && eventParams[selectedEvent || ""]?.[editIndex]?.default ? "Default Parametreyi Düzenle" : "Parametre Ekle"} width={360} onClose={handleDrawerClose} open={drawerOpen} bodyStyle={{ paddingBottom: 80 }} destroyOnClose>
          <Form layout='vertical' form={form} initialValues={{ key: "", value: "", description: "" }}>
            <Form.Item name='key' label='Parametre' rules={[{ required: true, message: "Parametre anahtarı gerekli" }]}>
              <Input placeholder='OM.uri, OM.oid, ...' disabled={editIndex !== null && eventParams[selectedEvent || ""]?.[editIndex]?.default} />
            </Form.Item>
            <Form.Item name='value' label='Değer' rules={[{ required: true, message: "Parametre değeri gerekli" }]}>
